...
if($_POST['check'] != '') {
	...
	$MailToAddress = "kbcamperswa@iinet.net.au"; // your email address
	$redirectURL = "http://kbcampers.com.au/thankyou.php"; // the URL of the thank you page.
	$MailSubject = "New mail from KB Campers website"; // the subject of the email
	...
	*****************************************************************/
	...
	if (!$email) {$email = $MailToAddress;}
		...
		$mailMessage .= "------=_NextPart_000_000E_01C5256B.0AEFE730--\r\n";
		$mailReturnPath .= "-f".$email;
	
		if (!mail($MailToAddress, $MailSubject, $mailMessage, $mailHeader, $mailReturnPath)) { echo "Error sending e-mail!";}
